...
- 2026-09-04: **Fixed production Vercel crash** — Slot component received multiple children instead of exactly one. Fixed 4 instances across `components/auth/auth-cta.tsx` and `components/settings/settings-content.tsx` where `Button asChild` passed icon elements + text nodes as separate children to Radix Slot, causing "Expected a single React element child or `Slottable`" error. Wrapped multi-child content in `<span>` elements to ensure exactly one child per Slot. Verified: typecheck, lint, and production build all pass.
